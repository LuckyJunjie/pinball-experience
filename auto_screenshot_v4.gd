...
func _initialize():
	print("=== Auto Screenshot Starting (Fixed v4) ===")
	...
	var menu = get_root().get_node("MainMenu")
	if menu:
		var play_btn = menu.find_child("PlayButton", true, false)
		if play_btn:
	...
	if menu:
		var char_btn = menu.find_child("Character2", true, false)
		if char_btn:
	...
	if menu:
		var start_btn = menu.find_child("DoneButton", true, false)
		if start_btn:
...
func capture_state(dir_path: String, state_name: String):
	...
	if err == OK:
		...
		print("ERROR: Failed to save " + full_path)
